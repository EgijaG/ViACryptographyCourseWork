...
def shouldDecrypt(value):
    global decryption
    decryption = value
    print("this is changing on button clicked:", value)
...
choiceText.pack()
aesBtn = tk.Button(text="AES", bg='blue', command = lambda: runFile('AES.py'))
desBtn = tk.Button(text="DES", bg='grey', command = lambda: runFile('DES.py'))
aesBtn.pack()
desBtn.pack()
