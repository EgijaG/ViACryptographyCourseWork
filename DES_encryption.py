...
def binaryToString(binary):
    ...
    for i in range(0, len(binary), 8):
        ...
        decimalString = decimalString + chr(decimal)
...
def shiftBitsLeft(k, nth_shifts):
    s = ""
...
def encrypt(pt, roundKeyBinary):
    ...
    for i in range(0, 16):
        #  Expansion permutation
        rightPartExpanded = permute(rightPart, expansionPermutation, 48)
 
        # XOR RoundKey[i] and right_expanded
        xorRoundKey = xor(rightPartExpanded, roundKeyBinary[i])
 
        # S-boxex: substituting the value from s-box table by calculating row and column
        sboxString = ""
        for j in range(0, 8):
            row = binaryToDec(int(xorRoundKey[j * 6] + xorRoundKey[j * 6 + 5]))
            col = binaryToDec(
                int(xorRoundKey[j * 6 + 1] + xorRoundKey[j * 6 + 2] + xorRoundKey[j * 6 + 3] + xorRoundKey[j * 6 + 4]))
            val = sBox[j][row][col]
            sboxString = sboxString + decToBinary(val)
 
        # Straight D-box: After substituting rearranging the bits
        sboxString = permute(sboxString, pBox, 32)
 
        # XOR left side with sboxString
        result = xor(leftPart, sboxString)
        leftPart = result
    ...
    combinedParts = leftPart + rightPart
 
    # Final permutation
    __binaryText = permute(combinedParts, finalPermutation, 64)
    return __binaryText
...
keyCompression = [14, 17, 11, 24, 1, 5,
            3, 28, 15, 6, 21, 10,
            23, 19, 12, 4, 26, 8,
            16, 7, 27, 20, 13, 2,
            41, 52, 31, 37, 47, 55,
